# -*- coding: utf-8 -*-
"""
Device driver for the Kurt J. Lesker KJLC ACG series capacitance manometer.
...
class KJLCACG(dev_generic.Device):
    """
    Device driver for Kurt J. Lesker KJLC ACG series capacitance manometers
    ...
    def get_message(self):
        ...
        if message is None:
            raise DeviceError(
                f'{self.device["Device"]}: No message could be found in the buffer')
        return message
    ...
    def check_errors(self, message):
        ...
        if error_byte & 0b11100111 != 0:
            raise DeviceError(
                f'{self.device["Device"]}: Error Message Received: {error_byte}')
